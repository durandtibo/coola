...
async def post_with_automatic_retry_async(
    ...
    **kwargs: Any,
    ...
    Example:
        ```pycon
        >>> import asyncio
        >>> from coola.experimental.http import post_with_automatic_retry_async
    ...
    try:
        return await _post_with_automatic_retry(
            client=client,
            url=url,
...
async def _post_with_automatic_retry(
    ...
    **kwargs: Any,
    ...
    Args:
        client: A httpx.AsyncClient object to use for making requests.
        url: The URL to send the POST request to.
        max_retries: Maximum number of retry attempts for failed requests.
            Must be >= 0.
        backoff_factor: Factor for exponential backoff between retries. The wait
    ...
    for attempt in range(max_retries + 1):
        try:
            response = await client.post(url=url, **kwargs)
