...
MODULE = "coola.utils.imports.pyarrow"
...
def test_check_pyarrow_with_package() -> None:
    with patch(f"{MODULE}.is_pyarrow_available", lambda: True):
        check_pyarrow()


def test_check_pyarrow_without_package() -> None:
    with (
        patch(f"{MODULE}.is_pyarrow_available", lambda: False),
...
def test_pyarrow_available_without_package() -> None:
    with patch(f"{MODULE}.is_pyarrow_available", lambda: False):
        fn = pyarrow_available(my_function)
        assert fn(2) is None
