...
def test_objects_are_equal_array() -> None:
    assert objects_are_equal(pandas.Series([1, 2, 3]), pandas.Series([1, 2, 3]))


@pandas_available
def test_pandas_array_equality_comparator_str() -> None:
    assert str(PandasSeriesEqualityComparator()).startswith("PandasSeriesEqualityComparator(")


@pandas_available
def test_pandas_array_equality_comparator__eq__true() -> None:
    assert PandasSeriesEqualityComparator() == PandasSeriesEqualityComparator()


@pandas_available
def test_pandas_array_equality_comparator__eq__false_different_type() -> None:
    assert PandasSeriesEqualityComparator() != 123


@pandas_available
def test_pandas_array_equality_comparator_clone() -> None:
...
@pandas_available
def test_pandas_array_equality_comparator_equal_true_same_object(config: EqualityConfig) -> None:
    series = pandas.Series([1, 2, 3])
    assert PandasSeriesEqualityComparator().equal(series, series, config)


@pandas_available
def test_pandas_array_equality_comparator_equal_true(
...
@pandas_available
def test_pandas_array_equality_comparator_equal_true_show_difference(
    caplog: pytest.LogCaptureFixture, config: EqualityConfig
) -> None:
...
@pandas_available
def test_pandas_array_equality_comparator_equal_false_different_value_show_difference(
    caplog: pytest.LogCaptureFixture, config: EqualityConfig
) -> None:
    config.show_difference = True
    comparator = PandasSeriesEqualityComparator()
...
@pandas_available
def test_pandas_array_equality_comparator_equal_false_different_type(
    caplog: pytest.LogCaptureFixture, config: EqualityConfig
) -> None:
    comparator = PandasSeriesEqualityComparator()
    with caplog.at_level(logging.INFO):
...
@pandas_available
def test_pandas_array_equality_comparator_equal_false_different_type_show_difference(
    caplog: pytest.LogCaptureFixture, config: EqualityConfig
) -> None:
    config.show_difference = True
    comparator = PandasSeriesEqualityComparator()
...
@pandas_available
def test_pandas_array_equality_comparator_equal_nan_false(config: EqualityConfig) -> None:
...
@pandas_available
def test_pandas_array_equality_comparator_equal_nan_true(config: EqualityConfig) -> None:
...
@pandas_available
def test_pandas_array_equality_comparator_no_pandas() -> None:
    with patch(
