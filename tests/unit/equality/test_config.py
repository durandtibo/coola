...
def test_equality_config_negative_atol() -> None:
    with pytest.raises(ValueError, match="Invalid EqualityConfig.*atol.*must be non-negative"):
        EqualityConfig(atol=-1.0)


def test_equality_config_negative_rtol() -> None:
    with pytest.raises(ValueError, match="Invalid EqualityConfig.*rtol.*must be non-negative"):
...
def test_equality_config_max_depth_zero() -> None:
    with pytest.raises(ValueError, match="Invalid EqualityConfig.*max_depth.*must be positive"):
