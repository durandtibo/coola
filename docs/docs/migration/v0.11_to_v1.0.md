...
result = objects_are_equal({"a": torch.tensor([1, 2, 3])}, {"a": torch.tensor([1, 2, 3])})
```
...
result = objects_are_allclose(np.array([1.0, 2.0]), np.array([1.0001, 2.0001]), atol=1e-3)
```
...
- Review the [examples](https://durandtibo.github.io/coola/uguide/equality/) for common usage patterns
- Open an issue on [GitHub](https://github.com/durandtibo/coola/issues) if you need help

## API Stability Note

⚠️ **Important**: This migration guide is for the 1.0 release series.
The current version is an alpha/pre-release.
